...
## Valor de G a usar no resto dos fits G = 3E6  --  G1 = resultado do fit de kittel
    ...
    for v in range(len(fwhm)):
        dh = fwhm[v] /( G_a_usar * np.sqrt(1+((G_a_usar*4* np.pi* Ms )/(2*peak[v]))**2))
        dH.append(dh)
    ...
    plt.text(peak[0],dH[-2],"Gilbert Damping ="+ str(valor),bbox=dict(facecolor='red', alpha=0.5))
    ...
    plt.text(fields[-5],dHa[0],"Gilbert Damping ="+ str(valorapp),bbox=dict(facecolor='red', alpha=0.5))
    ...
    file.write("x1_Peak, y1_dH, x2_newpeak, y2_dH_Fit, x3_Field, y3_dHa, y3_baseline"+"\n")
    for k in range(le):
        line=str(peak[k])+","+str(dH[k])+","+str(newpeak[k])+","+str(serie[k])+","+str(fields[k])+","+str(dHa[k])+","+str(baseline[0][k])
        file.write(line +"\n")
    file.close()   
